...
import { Search, X, MapPin, Play, Bell, Trash2, StopCircle, Tv, CircleDollarSign, MessageCircle, Send, Check, CheckCheck } from "lucide-react";
...
export function Header() {
  ...
  // Notifikasi & CS Chat
  const csNames = ["Tasya", "Felicia", "Ayu", "Nadia", "Siska", "Putri"];
  ...
  // Hook Fetching Data HARUS DI LUAR KONDISI (Aturan Mutlak React)
  const { isDramaBox, isReelShort, isShortMax, isNetShort, isMelolo, isFlickReels, isFreeReels, platformInfo } = usePlatform();
  const { data: dbRes } = useSearchDramas(isDramaBox ? nQuery : "");
  const { data: rsRes } = useReelShortSearch(isReelShort ? nQuery : "");
  const { data: nsRes } = useNetShortSearch(isNetShort ? nQuery : "");
  const { data: smRes } = useShortMaxSearch(isShortMax ? nQuery : "");
  const { data: mlRes } = useMeloloSearch(isMelolo ? nQuery : "");
  const { data: frRes } = useFlickReelsSearch(isFlickReels ? nQuery : "");
  const { data: freRes } = useFreeReelsSearch(isFreeReels ? nQuery : "");

  useEffect(() => {
    setIsMounted(true);
    setCsName("CS - " + csNames[Math.floor(Math.random() * csNames.length)]);
    ...
    const txtInt = setInterval(() => setTIdx(p => (p + 1) % 4), 3000);

    // Minta Akses Lokasi Langsung
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${pos.coords.latitude}&lon=${pos.coords.longitude}`).then(r=>r.json()).then(d=>setUserCity(d.address?.city||d.address?.town||d.address?.county||"Indonesia")).catch(()=>setUserCity("Indonesia")),
    ...
    // Jam Lengkap dengan AM/PM, Detik, Tahun
    ...
    // Initial Notif & CS Greeting
    const actN = [];
    const todayDate = new Date().toLocaleDateString('id-ID', {weekday:'long', day:'numeric', month:'short', year:'numeric'});
    const todayTime = new Date().toLocaleTimeString('en-US', {hour:'2-digit', minute:'2-digit', hour12:true});
    if (!localStorage.getItem('habi_yt_del')) {
      actN.push({ id:'yt', type:'youtube', time:`${todayDate} ${todayTime}`, title:'Admin Habi', text:'Dukung karya kami dengan {link} channel resmi kami.' });
    }
    setNotifs(actN);
    setChats([{ id:'c1', sender:'admin', time:todayTime, text:`Halo Kak! Saya CS dari tim Habi Music. Ada yang bisa dibantu hari ini? 😊` }]);

    const handleWd = (e) => {
      const d = e.detail;
      const t = new Date(d.timestamp).toLocaleTimeString('en-US', {hour:'2-digit', minute:'2-digit', hour12:true});
      setNotifs(p => [{ id:'wd_'+d.id, type:'withdraw', time:`${todayDate} ${t}`, title:'Finance Habi', text:`💳 PENDING: Rp100.000 ke ${d.method} (${d.account}) sedang diproses dlm 1-3 hari kerja.` }, ...p]);
    };
  ...
  // Logika Koin Nambah Hanya Pas Nonton dan Diam
  useEffect(() => {
    let cT, tT;
    if (promoState === 'progress' && isWatching && isIdle) {
      cT = setInterval(() => {
        setBalance(p => {
          ...
          setVideoToast(`+Rp ${e} masuk!`);
          clearTimeout(tT); tT = setTimeout(() => setVideoToast(null), 3000);
          return nB;
        });
      }, 25000); // Koin jalan tiap 25 detik selama idle di video
    }
    return () => { clearInterval(cT); clearTimeout(tT); };
  }, [promoState, isWatching, isIdle]);

  // Auto Scroll Chat
  useEffect(() => { if (chatRef.current) chatRef.current.scrollTop = chatRef.current.scrollHeight; }, [chats, chatOpen]);
  ...
  const sendChat = (e) => {
    ...
    // Waktu jeda baca & balas admin acak (realistis)
    const readDelay = Math.floor(Math.random() * 2000) + 1000; 
    const replyDelay = Math.floor(Math.random() * 15000) + 3000; 

    setTimeout(() => {
      setChats(p => p.map(m => m.id === msgId ? {...m, status:'read'} : m)); 
      setTimeout(() => {
        const l = userMsg.toLowerCase();
        let reply = "Baik Kak, pesan sudah kami terima. Mohon tunggu sebentar ya, tim kami akan segera mengeceknya 🙏";
        if(l.includes("cair") || l.includes("tarik") || l.includes("uang")) reply = "Untuk proses pencairan dana biasanya membutuhkan waktu 1-3 hari kerja ya Kak. Ditunggu saja ya 😊";
        else if(l.includes("koin") || l.includes("saldo") || l.includes("poin")) reply = "Koin akan otomatis bertambah saat Kakak memutar video drama tanpa di-scroll. Pastikan Kakak fokus menonton ya!";
        else if(l.includes("lama") || l.includes("belum")) reply = "Mohon maaf sekali atas keterlambatannya Kak 🙏 Saat ini antrean penarikan sedang padat. Kami akan proses secepatnya!";
  ...
  // Pemetaan Data Pencarian Aman
  const rawR = (isDramaBox ? dbRes : isReelShort ? rsRes?.data : isShortMax ? smRes?.data : isNetShort ? nsRes?.data : isMelolo ? mlRes?.data?.search_data?.flatMap(i=>i.books||[]).filter(b=>b.thumb_url) : isFlickReels ? frRes?.data : freRes) || [];
  const getMap = (i) => {
        ...
        <div className="container mx-auto px-4 flex items-center justify-between h-[60px]">
          ...
          <div className="flex items-center gap-1">
            <div className="relative">
              ...
              {showNotif && (
                <div className="absolute top-14 right-[-10px] sm:right-0 w-[320px] sm:w-[380px] bg-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.2)] border border-gray-100 z-50 overflow-hidden flex flex-col max-h-[80vh]">
                  <div className="flex justify-between items-center p-4 bg-gray-50 border-b border-gray-100">
                    <h3 className="font-black text-sm text-gray-800">{chatOpen ? csName : 'Pusat Notifikasi'}</h3>
                    <div className="flex gap-2">
                      {!chatOpen && <button onClick={()=>setChatOpen(true)} className="flex items-center gap-1 bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold hover:bg-green-200"><MessageCircle className="w-3 h-3"/> Chat CS</button>}
                      {chatOpen && <button onClick={()=>setChatOpen(false)} className="text-xs font-bold text-gray-500 hover:text-gray-800 mr-2">Kembali</button>}
                      <button onClick={() => setShowNotif(false)}><X className="w-5 h-5 text-gray-500" /></button>
                    </div>
                  </div>

                  <div className="flex-1 overflow-y-auto bg-white p-2" ref={chatRef}>
                    {!chatOpen ? (
                      notifs.length > 0 ? notifs.map(n => (
                        <div key={n.id} className={`flex gap-3 p-3 rounded-xl mb-2 border ${n.type==='withdraw'?'bg-blue-50 border-blue-100':'bg-white border-gray-100 shadow-sm'}`}>
                          <div className={`w-8 h-8 rounded-full flex justify-center items-center flex-shrink-0 mt-1 text-white font-bold text-xs ${n.type==='withdraw'?'bg-blue-500':'bg-[#FF0000]'}`}>{n.type==='withdraw'?'Rp':'HM'}</div>
                          <div className="flex-1 pr-2">
                            <div className="flex justify-between items-start mb-1"><span className="font-bold text-[11px] text-gray-900">{n.title}</span><span className="text-[9px] font-bold text-gray-400">{n.time}</span></div>
                          ...
                          <button onClick={()=>delN(n.id)} className="text-gray-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      )) : <div className="py-10 text-center text-gray-400 text-sm font-medium">Belum ada notifikasi baru.</div>
                    ) : (
                      <div className="flex flex-col gap-3 p-2">
                        <div className="text-center text-[10px] text-gray-400 font-medium mb-2">Hari ini</div>
                        {chats.map(c => (
                          <div key={c.id} className={`flex flex-col max-w-[85%] ${c.sender === 'user' ? 'self-end items-end' : 'self-start items-start'}`}>
                            {c.sender === 'admin' && <span className="text-[10px] text-gray-500 mb-1 ml-1 font-bold">{csName}</span>}
                            <div className={`p-3 rounded-2xl text-xs font-medium shadow-sm ${c.sender === 'user' ? 'bg-green-100 text-green-900 rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none'}`}>{c.text}</div>
                            <div className="flex items-center gap-1 mt-1 mr-1">
                              <span className="text-[9px] text-gray-400">{c.time}</span>
                              {c.sender === 'user' && (c.status === 'read' ? <CheckCheck className="w-3 h-3 text-blue-500"/> : <Check className="w-3 h-3 text-gray-400"/>)}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {chatOpen && (
                    <form onSubmit={sendChat} className="p-3 bg-white border-t border-gray-100 flex gap-2">
                      <input type="text" value={chatInput} onChange={e=>setChatInput(e.target.value)} placeholder="Ketik pesan..." className="flex-1 bg-gray-100 rounded-full px-4 py-2 text-xs outline-none focus:ring-1 focus:ring-green-400"/>
                      <button type="submit" className="bg-green-500 text-white p-2 rounded-full hover:bg-green-600"><Send className="w-4 h-4"/></button>
                    </form>
                  )}
                </div>
              )}
